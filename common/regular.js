...
export function isFloatTowMoreThanZero(number) {
  const no = Number(number)
  if (no) {
    return no > 0 && isFloatForTwo(no)
  } else {
    return false
  }
}
...
export function compareSize(number1, number2) {
  const no1 = parseFloat(number1)
  const no2 = parseFloat(number2)
  return no1 >= no2
}
