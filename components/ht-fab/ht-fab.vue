<template>
  <view class="uni-cursor-point ">
    <view
      v-if="popMenu && (leftBottom || rightBottom || leftTop || rightTop) && 0 < content.length && showMenu"
      :class="{
      ...
      class="lc-fab-touch "
      :style="{ top: contTop2 + 'px', left: contLeft2 }"
      style="height: 55px;border: 0px solid green;"
    >
      <view
        :class="{
        ...
        class="lc-fab-touch__content"
        elevation="5"
        style="border: 0px solid red;"
      >
        <view v-if="flexDirectionStart || horizontalLeft" class="lc-fab-touch__item lc-fab-touch__item--first" />
        <view v-for="(item, index) in content" :key="index" :class="{ 'lc-fab-touch__item--active': isShow }" class="lc-fab-touch__item" @click="_onItemClick(index, item)">
    ...
    <view
      ...
      class="lc-fab-touch__circle lc-fab-touch__plus drag-ball-box"
      :style="{ 'background-color': styles.buttonColor, left: left + 'px', top: top + 'px' }"
      @click="_onClick"
      id="drag-ball"
...
export default {
  ...
  methods: {
            ...
            dragBallSize => {
              ...
              uni.getSystemInfo({
                success: function(res) {
                  ...
                  const marginBottom = marginRight * 2;
                  that.bottom = parseInt(dragBallSize.height + marginBottom);
                  that.right = parseInt(dragBallSize.width + marginRight);
                  that.safeArea.minTop = marginBottom;
                  that.safeArea.maxTop = res.windowHeight - that.bottom;
                  that.safeArea.minLeft = marginRight;
                  that.safeArea.maxLeft = res.windowWidth - that.right;
    ...
    dragMove(event) {
      ...
      // 拖出屏幕
      if (this.left < this.right || this.top < this.bottom || this.left > this.safeArea.windowWidth || this.top > this.safeArea.windowHeight) {
        this.dragEnd();
      }
    },
    // 手指触摸悬浮球动作结束，贴边并延迟隐藏
