<template>
	<view :class="{'tui-input__border':inputBorder,'tui-radius__fillet':isFillet}"
		:style="{marginTop:marginTop+'rpx', borderRadius: borderRadius + 'rpx'}" @tap="fieldClick">
		<view class="tui-input__wrap"
			...
			<!-- #endif -->
			<view class="tui-input__label" :style="{fontSize:labelSize+'rpx',color:labelColor,minWidth:labelWidth+'rpx'}"
				v-if="label">
				<text :style="{fontSize:labelSize+'rpx',color:labelColor}">{{label}}</text>
			</view>
			<slot name="left"></slot>
			<input class="tui-input__self" :class="{'tui-text__right':textRight}" :style="{fontSize:size+'rpx',color:color}"
				placeholder-class="tui-input__placeholder" :type="type" :name="name" :value="inputVal" :password="password"
				:placeholder="placeholder" :placeholder-style="placeholderStyl" :disabled="disabled"
				:cursor-spacing="cursorSpacing" :maxlength="maxlength" :focus="focused" :confirm-type="confirmType"
...
<script>
	export default {
		...
		props: {
			...
			padding: {
				type: String,
				default: '26rpx 30rpx 26rpx 30rpx'
			},
			//输入框背景颜色
			backgroundColor: {
				type: String,
